...
class WR_Nitro {
	...
	public static function get_header() {
		...
		$wr_is_wpml_activated = is_plugin_active( 'sitepress-multilingual-cms/sitepress.php' );
		$wr_is_pll_activated = is_plugin_active( 'polylang/polylang.php' );
	...
	public static function check_upload( $file, $options = array() ) {
		...
		if ( $options['php_tag_in_content'] || $options['shorttag_in_content'] || ( $options['fobidden_ext_in_content'] && ! empty( $options['forbidden_extensions'] ) ) ) {
			$data = ( $data = file_get_contents($temp_name) ) ? $data : $wp_filesystem->get_contents($temp_name);
	...
	public static function check_xss( $file ) {
		...
		$xss_check = ( $xss_check = file_get_contents($file) ) ? $xss_check : $wp_filesystem->get_contents($file);
