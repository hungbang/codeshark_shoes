...
class WR_Nitro_Render {
	...
	public static function custom_styles() {
		...
		$buddypress_activated = ( function_exists( 'is_plugin_active' ) && is_plugin_active( 'buddypress/bp-loader.php' ) && class_exists( 'BuddyPress' ) );
		...
		if( get_query_var( 'taxonomy' ) == 'product_cat' ) {
			...
			$image_banner_id = absint( get_woocommerce_term_meta( $categories['term_id'], 'image_banner_id', true ) );

			if( $image_banner_id ) {
				$image_url = wp_get_attachment_url( $image_banner_id );

				if( $image_url ) {
					$wr_background_size = esc_attr( get_woocommerce_term_meta( $categories['term_id'], 'wr_background_size', true ) );
					$wr_background_size = $wr_background_size ? $wr_background_size : 'auto';

					$wr_background_repeat = esc_attr( get_woocommerce_term_meta( $categories['term_id'], 'wr_background_repeat', true ) );
					$wr_background_repeat = $wr_background_repeat ? $wr_background_repeat : 'no-repeat';

					$wr_background_position = esc_attr( get_woocommerce_term_meta( $categories['term_id'], 'wr_background_position', true ) );
					$wr_background_position = $wr_background_position ? $wr_background_position : 'left top';

					$wr_background_attachment = esc_attr( get_woocommerce_term_meta( $categories['term_id'], 'wr_background_attachment', true ) );
...
CSS;
	...
	public static function body_class( $classes ) {
		...
		if ( function_exists( 'is_plugin_active' ) && is_plugin_active( 'js_composer/js_composer.php' ) && $wr_enable_page_builder == 'true' ) {
