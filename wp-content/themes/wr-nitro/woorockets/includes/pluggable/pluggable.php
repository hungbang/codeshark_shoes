...
class WR_Nitro_Pluggable {
	...
	public static function plugins_wp_super_cache() {
		...
		if ( ! file_exists( $wp_cache_link ) ) {
			...
			$content = ( $content = file_get_contents($wp_cache_file) ) ? $content : $wp_filesystem->get_contents($wp_cache_file);

			file_put_contents($wp_cache_link, $content) || $wp_filesystem->put_contents($wp_cache_link, $content);
		}
	}
	...
	public static function set_user_agent() {
		if( ! empty( $_SERVER['HTTP_USER_AGENT'] ) && ! empty( $_REQUEST['wp_customize'] ) && $_REQUEST['wp_customize'] == 'on' && ! empty( $_REQUEST['customized'] ) ) {
