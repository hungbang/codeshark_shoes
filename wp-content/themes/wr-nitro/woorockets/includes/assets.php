...
class WR_Nitro_Assets {
	...
	public static function init() {
		...
		add_action( 'wp_enqueue_scripts', array( __CLASS__, 'optimize_site_assets' ), 9999999997 );
		...
		if ( ( int ) $wr_nitro_options['compress_css'] ) {
			...
			add_action( 'wp_enqueue_scripts', array( __CLASS__, 'compress_site_styles'  ), 9999999998 );
		}

		if ( ( int ) $wr_nitro_options['compress_js'] ) {
			// Register actions to compress and combine scripts.
			add_action( 'wp_enqueue_scripts', array( __CLASS__, 'compress_site_scripts' ), 9999999999 );
...
//			wp_dequeue_script( 'woocommerce' );
	...
	public static function compress_site_styles() {
		...
		foreach ( $wp_styles->to_do as $handle ) {
			...
			// Do not compress stylesheet of Ninja Popup.
			...
			if ( ! @is_file( $compression ) || filemtime( $compression ) < $last_modification[ $handle ] ) {
				...
				foreach ( $sources as $source ) {
					...
					if ( strlen( $compressed ) >= $max_size ) {
						if ( $num_files == 1 ) {
							if ( ! ( file_put_contents($compression, $compressed) || $wp_filesystem->put_contents($compression, $compressed) ) ) {
								return;
							}
						} else {
							$nextFile = substr($compression, 0, -4) . "-{$num_files}.css";

							if ( ! ( file_put_contents($nextFile, $compressed) || $wp_filesystem->put_contents($nextFile, $compressed) ) ) {
								return;
							}
						}
			...
			if ( ! @is_file( $compression ) || filemtime( $compression ) < $last_modification[ $handle ] ) {
				...
				foreach ( $sources as $source ) {
					...
					if ( false !== strpos( $source, '.min.js' ) ) {
						$compressed .= ( $buffer = file_get_contents(ABSPATH . $source) ) ? $buffer : $wp_filesystem->get_contents(ABSPATH . $source);
					} else {
						$compressed .= JShrink_Minifier::minify(
							( $buffer = file_get_contents(ABSPATH . $source) ) ? $buffer : $wp_filesystem->get_contents(ABSPATH . $source),
	...
	public static function get_location() {
		...
		if ( ! @is_dir( $path ) ) {
			...
			if ( ! ( mkdir($path) || $wp_filesystem->mkdir($path) ) ) {
				return false;
			}
		}
	...
	public static function minify_css( $stylesheet ) {
		...
		$css = ( is_file($stylesheet) || $wp_filesystem->is_file($stylesheet) )
			? ( ( $buffer = file_get_contents($stylesheet) ) ? $buffer : $wp_filesystem->get_contents($stylesheet) )
			: $stylesheet;
