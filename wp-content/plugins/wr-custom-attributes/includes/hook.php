            ...
            >
				...
				<option
                    value="<?php echo esc_attr(
                        version_compare(WC_VERSION, '3.0.0', 'lt')
                        ? $term->slug
                        : $term->term_id
                    ); ?>"
                    <?php
                    $selected = selected(has_term(absint($term->term_id), $taxonomy, 0), true, false);

                    if (empty($selected)) {
                        // Try another method to check if the current attribute value is selected.
                        if (empty($options)) {
	                        // Get product attribute options.
	                        $id      = absint( $_POST['post_id'] );
	                        $type    = ! empty( $_POST['product_type'] ) ? wc_clean( $_POST['product_type'] ) : 'simple';
	                        ...
	                        if ( isset( $attrs[ $taxonomy ] ) ) {
		                        $options = $attrs[ $taxonomy ]->get_options();
		                        $options = empty( $options ) ? array() : $options;
	                        }
                        }

	                    if (!empty($options)) {
                            $selected = wc_selected( $term->term_id, $options );
	                    }
                    }
